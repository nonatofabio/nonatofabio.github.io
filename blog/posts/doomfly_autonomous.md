...
Now, here is the thing: I did not write the code, the infrastructure or the first drafts of the docs. All of it came out of the [Strands harness](https://github.com/strands-agents/harness-sdk), an open-source coding agent from the Strands Agents team at AWS that you run from the terminal. It keeps its own state on disk between sessions and runs tools, tests and cloud deployments on its own. Here it wrote the pipeline that turns the fly's wiring into a network, the five ordinary Doom agents that served as teachers, the trainer that taught the fly-wired network to copy them, and the second training stage that tried to push it further. It also wrote the cloud setup that ran a GPU fleet in three AWS regions, and it recorded the footage. Five days, 28 commits, about 3,300 lines of Python and shell, twelve fine-tuning runs and three control runs.
...
The code is public at [nonatofabio/doomfly-rl](https://github.com/nonatofabio/doomfly-rl), and the trained checkpoints are on [Hugging Face](https://huggingface.co/fabiononato/doomfly-rl) with the connectome file they need. If you want to watch a network shaped like a fly play Doom, start there.
